...
class BERTDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        c1 = (
            ...
            + [self.vocab.stoi.get(c, self.vocab.unk_index) for c in c1.split()]
            + [self.vocab.eos_index]
        )
        c2 = [self.vocab.stoi.get(c, self.vocab.unk_index) for c in c2.split()] + [
            self.vocab.eos_index
        ]
        ...
        dfg_segment_label = ([1 for _ in range(len(d1))] + [2 for _ in range(len(d2))])[
            : self.seq_len
        ]
        cfg_segment_label = ([1 for _ in range(len(c1))] + [2 for _ in range(len(c2))])[
            : self.seq_len
        ]
        ...
        padding = [
            self.vocab.pad_index for _ in range(self.seq_len - len(dfg_bert_input))
        ]
        dfg_bert_input.extend(padding), dfg_bert_label.extend(
            padding
        ), dfg_segment_label.extend(
            padding
        )  # , comp_label.extend(padding)
        cfg_padding = [
            self.vocab.pad_index for _ in range(self.seq_len - len(cfg_bert_input))
        ]
    ...
    def get_corpus_line(self, item):
        if self.on_memory:
            return (
                self.cfg_lines[item][0],
                self.cfg_lines[item][1],
                self.dfg_lines[item][0],
                self.dfg_lines[item][1],
            )
